...
/* ── Section file download (client-side ZIP via JSZip CDN) ── */
...
async function downloadSectionFiles(section) {
  if (!section || !section.items || section.items.length === 0) {
    alert('Δεν υπάρχουν κινήσεις σε αυτό το section.')
    return
  }

  const txnIds = section.items.map(i => i.id).filter(Boolean)
  if (txnIds.length === 0) {
  ...
  for (const id of txnIds) {
    try {
      ...
      if (files.length > 0) txnsWithFiles++
      files.forEach(f => {
  ...
  const fetchPromises = allFiles.map(async (f) => {
    try {
      ...
      // Avoid filename collisions (same filename in different txns)
      let name = f.name
  ...
  // Step 3: build ZIP and trigger download
